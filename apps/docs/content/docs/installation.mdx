---
title: Installation
description: Install the BProgress for React utility in your application
---

import { Tab, Tabs } from 'fumadocs-ui/components/tabs';
import {
  SiReact,
  SiNextdotjs,
  SiNpm,
  SiRemix,
  SiVuedotjs,
} from 'react-icons/si';
import { HiDotsHorizontal } from 'react-icons/hi';
import { FaGithub } from 'react-icons/fa6';

## Installation

### Node.js

To install BProgress, run the following command:

<Tabs groupId="packageManager" items={['npm', 'pnpm', 'yarn', 'bun']} persist>
  <Tab value="npm">```bash npm install @lop-town/bprogress-core ```</Tab>
  <Tab value="pnpm">```bash pnpm add @lop-town/bprogress-core ```</Tab>
  <Tab value="yarn">```bash yarn add @lop-town/bprogress-core ```</Tab>
  <Tab value="bun">```bash bun add @lop-town/bprogress-core ```</Tab>
</Tabs>

### CDN

Use **unpkg**:

- JS: https://unpkg.com/@lop-town/bprogress-core/dist/index.global.js
- ESM: https://unpkg.com/@lop-town/bprogress-core/dist/index.js
- CSS: https://unpkg.com/@lop-town/bprogress-core/dist/index.css

Use **jsDelivr**:

- JS: https://cdn.jsdelivr.net/npm/@lop-town/bprogress-core/dist/index.global.js
- ESM: https://cdn.jsdelivr.net/npm/@lop-town/bprogress-core/dist/index.js
- CSS: https://cdn.jsdelivr.net/npm/@lop-town/bprogress-core/dist/index.css

### Integrations Libraries

The following frameworks have a dedicated package for using BProgress:

<Cards>
  <Card title="React" href="/docs/react/installation" icon={<SiReact />}>
    Install the BProgress utility for React
  </Card>
  <Card title="Next.js" href="/docs/next/installation" icon={<SiNextdotjs />}>
    Install BProgress in your Next.js application
  </Card>
  <Card title="Remix" href="/docs/remix/installation" icon={<SiRemix />}>
    Install BProgress in your Remix application
  </Card>
  <Card title="Vue" href="/docs/vue/installation" icon={<SiVuedotjs />}>
    Install the BProgress utility for Vue
  </Card>
  <Card title="Soon..." icon={<HiDotsHorizontal />}>
    More integrations coming soon...
  </Card>
</Cards>

## Links

<Cards>
  <Card
    external
    title="NPM"
    href="https://www.npmjs.com/package/@lop-town/bprogress-core"
    icon={<SiNpm />}
  >
    Check the package on NPM
  </Card>
  <Card
    external
    title="GitHub"
    href="https://github.com/lop-town/bprogress/tree/main/packages/core"
    icon={<FaGithub />}
  >
    Check the repository on GitHub
  </Card>
</Cards>
