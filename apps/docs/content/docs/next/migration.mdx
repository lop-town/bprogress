...
**`next-nprogress-bar` becomes `@lop-town/bprogress-next`!**
...
You need to uninstall `next-nprogress-bar` and install `@lop-town/bprogress-next`.

<Tabs groupId="packageManager" items={['npm', 'pnpm', 'yarn', 'bun']} persist>
  <Tab value="npm">
    ```diff - npm uninstall next-nprogress-bar + npm install @lop-town/bprogress-next ```
  </Tab>
  <Tab value="pnpm">
    ```diff - pnpm remove next-nprogress-bar + pnpm add @lop-town/bprogress-next ```
  </Tab>
  <Tab value="yarn">
    ```diff - yarn remove next-nprogress-bar + yarn add @lop-town/bprogress-next ```
  </Tab>
  <Tab value="bun">
    ```diff - bun remove next-nprogress-bar + bun add @lop-town/bprogress-next ```
  </Tab>
</Tabs>
...
+ import { ... } from '@lop-town/bprogress-next';
```
...
If you encounter any issues, please [open an issue](https://github.com/lop-town/bprogress/issues) on the BProgress repository. We'll be happy to help you as soon as possible!
